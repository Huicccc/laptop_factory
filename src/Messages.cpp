...
void LaptopOrder::Marshal(char *buffer) {
    // // Host TO Network Long, converts the unsigned integer from host byte order to network byte order
    int net_customer_id = htonl(customer_id);
    int net_order_number = htonl(order_number);
    int net_laptop_type = htonl(laptop_type);
    // memcpy, copy each network byte order integer into the buffer at the correct position, indicated by offset
    int offset = 0;
